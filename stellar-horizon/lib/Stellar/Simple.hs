      ...
      where
        ...
        transaction'memo =
            case memo of
                ...
                MemoText t ->
                    XDR.Memo'MEMO_TEXT $ XDR.lengthArray' $ encodeUtf8 t
                MemoOther{} -> undefined

        transaction'operations =
            XDR.boundLengthArrayFromList $ toList operations
            ...
            operations
                |> XDR.Operation
                    { operation'sourceAccount =
                        XDR.PublicKey'PUBLIC_KEY_TYPE_ED25519 . addressToXdr
                            <$> msource
                    , operation'body
                    }
        , ..
...
op_accountMerge ::
    "destination" :! Address -> TransactionBuilder -> TransactionBuilder
op_accountMerge (Arg destination) =
    addOperation Nothing $
        XDR.OperationBody'ACCOUNT_MERGE $
            XDR.MuxedAccount'KEY_TYPE_ED25519 $
                addressToXdr destination
...
op_changeTrust ::
    ...
    TransactionBuilder ->
    TransactionBuilder
op_changeTrust (Arg line) (Arg limit) =
    addOperation Nothing $
        XDR.OperationBody'CHANGE_TRUST
            XDR.ChangeTrustOp
                { changeTrustOp'line = assetToXdr line
                , changeTrustOp'limit = limit
                }

op_deleteTrust :: "line" :! Asset -> TransactionBuilder -> TransactionBuilder
op_deleteTrust = op_changeTrust ! #limit 0
...
signWithSecret secret tx =
    either (error . show) identity $
        StellarSignature.signTx
            publicNetwork
            tx
            [StellarKey.fromPrivateKey' secret]
...
op_setHomeDomain domain =
    addOperation Nothing $ XDR.OperationBody'SET_OPTIONS options
  where
...
op_setSigners =
    appEndo
        . Map.foldMapWithKey \address weight -> Endo $ setSigner address weight
  where
    setSigner address weight = addOperation Nothing body
      where
        body =
            XDR.OperationBody'SET_OPTIONS
                defaultOptions{XDR.setOptionsOp'signer}
        setOptionsOp'signer = Just XDR.Signer{signer'key, signer'weight}
        signer'key =
