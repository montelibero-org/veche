...
--
-- Since: yesod-form 1.3.8
renderBootstrap5 :: Monad m => BootstrapFormLayout -> FormRender m a
              ...
              <div .d-sm-flex d-grid :has $ fvErrors view:.has-error>
                $case formLayout
                  ...
                  $of BootstrapHorizontalForm labelOffset labelSize inputOffset inputSize
                    $if fvId view /= bootstrapSubmitId
                      <label :Blaze.null (fvLabel view):.sr-only .col-form-label .fw-bold .#{toOffset labelOffset} .#{toColumn labelSize} for=#{fvId view}>#{fvLabel view}
                      <div .mb-3 .#{toOffset inputOffset} .#{toColumn inputSize}>
                        ^{fvInput view}
                        ^{helpWidget view}
                    $else
                      <div .mb-3 .#{toOffset (addGO inputOffset (addGO labelOffset labelSize))} .#{toColumn inputSize}>
                        ^{fvInput view}
                        ^{helpWidget view}
                |]
    return (res, widget)
...
-- as useful since you're not going to use 'renderBootstrap5'
-- anyway.
--
-- Since: yesod-form 1.3.8
mbootstrapSubmit
...
--
-- That form can then be rendered into a widget using the 'renderBootstrap5' function. Here, the form is laid out vertically using 'BootstrapBasicForm':
--
-- > (formWidget, formEnctype) <- generateFormPost $ renderBootstrap5 BootstrapBasicForm personForm
--
...
--
-- > (formWidget, formEnctype) <- generateFormPost $ renderBootstrap5 (BootstrapHorizontalForm (ColSm 0) (ColSm 4) (ColSm 0) (ColSm 6)) personForm
--
